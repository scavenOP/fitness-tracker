...
export class TrackerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private async centerOnCurrentPosition() {
    try {
      if (this.isNative) {
    ...
    if (this.isNative) {
      try {
        ...
        if (perm.location !== 'granted' && perm.coarseLocation !== 'granted') {
          alert('Location permission is required to track your activity.');
          return;
        }
      } catch (_) {}
    }
    this.routeLine?.setLatLngs([]);
    await this.tracking.startTracking(this.actType() as 'walk' | 'run');
